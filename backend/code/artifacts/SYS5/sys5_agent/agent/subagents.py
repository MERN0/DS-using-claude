...
def build_subagents(input_root: Path) -> list[dict]:
    ...
    resolution_agent = {
        ...
        "system_prompt": (
            ...
            "pass the exact `file_name` recorded in discovery.md straight "
            "into these tools' file_name argument, never a constructed path; "
            "this run's input directory is fixed and the tools already know "
            "it.\n\n"
    ...
    test_case_drafting_agent = {
        ...
        "system_prompt": (
            ...
            "Priority) -- Test Case ID is overwritten automatically at write "
            "time (see output-format), so any placeholder there is fine; "
            "don't spend effort on it. Check Type is the cluster's "
            "check_type -- use writing-style's check-type-driven wording "
            "section to decide what values/steps fit that type (e.g. "
            "boundary values for Boundary Value Check, an out-of-range value "
            "plus the resolved rejection response for Invalid Values "
            "Check).\n\n"
    ...
    qa_validation_agent = {
        ...
        "description": (
            ...
            "completeness, the fixed SET/VERIFY operator syntax, valid "
            "Check Type values, consistent 1./2./3. numbering with a "
            "matching Expected Result line for every Test Step, and whether "
            "extraction actually covered the full requirements sheet. Call "
            "once after all clusters have been drafted, and again after any "
            "re-draft/re-resolve retry."
        ),
        "system_prompt": (
            ...
            "clusters.jsonl, discovery.md, and every resolved/*.md file from "
            "the run workspace.\n\n"
